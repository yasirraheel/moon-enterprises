...
public class WithdrawalMethodAdapter extends ArrayAdapter<WithdrawalMethod> {
    
    private final Context context;
    private final List<WithdrawalMethod> methods;
    private final boolean showPlaceholder;
    
    public WithdrawalMethodAdapter(@NonNull Context context, List<WithdrawalMethod> methods, boolean showPlaceholder) {
    ...
    @Nullable
    @Override
    public WithdrawalMethod getItem(int position) {
    ...
    private View createView(int position, View convertView, ViewGroup parent, int layoutResource) {
        ...
        WithdrawalMethod method = getItem(position);
        
        if (method == null) {
            // Placeholder item - hide the image
            holder.textView.setText("Select Withdrawal Method");
            holder.imageView.setVisibility(android.view.View.GONE);
        } else {
            ...
            if (method.getBankImage() != null && !method.getBankImage().isEmpty()) {
                ...
                android.util.Log.d("WithdrawalAdapter", "Loading spinner image: " + imageUrl);
